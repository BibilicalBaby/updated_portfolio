    ...
    return (
        <>
            <div className='relative'>
                {
                    !isOpen &&
                    <div className='flex items-center justify-between py-5 px-7 md:hidden'>
                        <IoIosMenu onClick={toggleMenu} className='z-50 text-2xl font-bold' />
                        <a href='#' className='font-sniglet font-light'>&lt;muthusi_dev/&gt;</a>
                    </div>
                }
                {isOpen && (
                    <div className={`flex md:flex-row md:items-center fixed md:h-auto top-0 left-0 flex-col md:w-full h-full w-1/2 md:bg-white text-black z-20 bg-[whitesmoke] md:py-4 md:justify-between  md:px-7`}>
                        <IoMdClose className='absolute top-2 right-3 text-white font-bold text-xl md:hidden' onClick={toggleMenu} />
                        <a href='#home' id='home' onClick={handleScroll} className={`font-sniglet font-light hidden md:inline ${activeSection === 'home' ? 'text-blue-600 font-roboto scale-105 duration-300 transition  ease-in' : ''}`}>&lt;muthusi_dev/&gt;</a>
                        <div className='md:flex md:flex-row md:justify-between md:gap-10 h-full'>
                            ...
                            <div id="what_i_do" className={`flex items-center gap-1 md:font-poppins font-sniglet pl-4 md:pl-0 my-10 md:my-0 relative md:after:hidden  after:-bottom-3 after:block after:absolute  after:h-px after:w-[90%] after:bg-black after:opacity-25 ${activeSection === 'what_i_do' ? 'text-blue-600 font-roboto scale-105 duration-300 transition  ease-in' : ''}`}>
                                <FaPersonDigging />
                                <a href="#what_i_do" onClick={handleScroll}>what i do</a>
                            </div>
                            <div id='mySkills' className={`flex items-center gap-1 md:font-poppins font-sniglet pl-4 md:pl-0 my-10 md:my-0 relative md:after:hidden  after:-bottom-3 after:block after:absolute  after:h-px after:w-[90%] after:bg-black after:opacity-25 ${activeSection === 'mySkills' ? 'text-blue-600 font-roboto scale-105 duration-300 transition  ease-in' : ''}`}>
                                <IoCodeSlashOutline />
